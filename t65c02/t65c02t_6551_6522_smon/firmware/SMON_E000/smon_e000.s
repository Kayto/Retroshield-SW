...
;;;
;;; -------------------------------------------------------------------------
;;; SMON_E000 by kayto@github.com (2025)
;;;
;;; This is a version of SMON for the Retroshield.
;;;
;;; Hardware:
;;;   - UART: WDC 6551 @ $8400, 19200 baud
;;;   - VIA1: $9000, VIA2: $8800
;;;   - RAM: 32K ($0000-$7FFF)
;;;   - ROM: 8K @ $E000
;;;
;;; Build System:
;;;   - ca65/ld65 toolchain (converted from VASM)
;;;   - Linker config: smon_e000.cfg
;;;   - Output: 8K binary (no padding)
;;;
;;; Boot Flow:
;;;   - ENTRY: Cold boot - init stack, set vectors, print banner, prompt
;;;   - SMON:  BRK entry - save registers, show 'R' command, prompt
;;;   - X cmd: Cold restart (jmp ENTRY)
;;;
;;; UART:
...
;;;
;;; Help System:
;;;   - Categorized: H A, H M, H F, H E, H C, H R, H O
;;;
;;; Commands:
;;;   - X: Cold restart (reset monitor)
...
;;; =========================================================================
;;; ENTRY - Cold boot entry point (RESET vector points here)
;;; =========================================================================
ENTRY:
        cld                     ; Clear decimal mode
        ldx     #$FF            ; Initialize stack pointer
        txs
        lda     #<SMON          ; Set BRK vector to SMON
        sta     BRK_LO
        lda     #>SMON    
        sta     BRK_HI
        lda     #$00            ; Initialize saved registers
        sta     PCLSAVE
        sta     PCHSAVE
        sta     SRSAVE
        sta     AKSAVE
        ...
        lda     #$FF
        sta     SPSAVE
        jsr     welcome         ; Print welcome banner
        lda     #'R'            ; Execute 'R' command to show registers
        jmp     LC2FF           ; Enter command loop with 'R' command
...
HLPMSG_OTHER:
        ...
        .byte  $0d
        .byte   "X               - Cold restart (reset monitor)"
        .byte  $0d
...
welmsg:
        ...
        .byte $0D
        .byte "Modified for Retroshield by kayto@github.com"
        .byte $0D
...
;;; =========================================================================
;;; SMON - BRK instruction entry point
;;; CPU pushes PCH, PCL, SR then jumps here via BRK vector
;;; =========================================================================
SMON:   cld
        ldx     #$05
@save_regs:
        pla
        sta     PCHSAVE,x       ; Save PCH, PCL, SR, A, X, Y from stack
        dex
        bpl     @save_regs
        ;; Adjust PC (BRK pushes PC+2, we want PC+1)
        lda     PCLSAVE
        bne     @no_borrow
        dec     PCHSAVE
@no_borrow:
...
;;; =========================================================================
;;; SHELLCMD_MON - X command: Cold restart (reinitialize and show banner)
;;; =========================================================================
SHELLCMD_MON:
        jmp     ENTRY           ; Full cold restart

;;; Helper for GO command - load saved SP into X
LB7B2:  lda     SPSAVE
        tax
